...
to do-actions                                ;; peer procedure: it's the main procedure used by the active peer for downloading then ranking, and liking documents
  ...
  ifelse length doc-list >= top-k [
   ...
   if doc-list != [] [
    ...
    [ifelse (score-metric = "Andre Score")
      [calculate-score-andre doc-list]
...
;;THIS FUNCTION IS REALLY MESSY - it's way too hard to read
...
to calculate-score [ranked-list]   ;;procedure for calculating the score for the active peer using Hachemi's method
   ...
   ifelse (result >= 1) [                  ;; to increase the score we count the hits, in this case for a document to be considered a hit it has to share at least one label (tag) with the the peer (taste)
    ...
    [
      ...
      set score score - 1                                              ;; note here that this utility function is arbitrary and can be replaced by a more significant and meaningful one
      show "EXECUTED!!!!"
      show ranked-list
...
to calculate-score-andre [ranked-list]
  if verbose? [ show word "Past score: " score ]
 
  foreach ranked-list [
   let matched-taste false
   let tempTaste taste
   let tempTag [tag] of ?
...
score-metric
score-metric
"Babak Score" "Hachemi Score" "Andre Score"
1
...
"" ""
PENS
"peer follower" 1.0 0 -2674135 true "" "if (ticks mod 5 = 0) [plot mean [score] of peers with [population = \"crowd follower\" and count out-follow-link-neighbors > 0]]"
"pen-1" 1.0 0 -13791810 true "" "if ticks mod 5 = 0 [plot mean [score] of peers with [population = \"friend follower\" and count out-follow-link-neighbors > 0]]"
"pen-2" 1.0 0 -7500403 true "" "if ticks mod 5 = 0 [plot mean [score] of peers with [population = \"attacker\"]]"
...
582
Attacker ascent
(mean [score] of peers with [population = \"attacker\"]) / ticks
17
1
11
...
129
629
crowd follower ascent
(mean [score] of peers with [population = \"crowd follower\"]) / ticks
17
1
11
...
142
675
peer follower ascent
(mean [score] of peers with [population = \"friend follower\"]) / ticks
17
1
11
...
We have the switch "dynamic_network" which if set at ON, will enable peers to join and disconnect from the network, it's important to note that a disconnected peer cut off all connection, and then when he's online again he restores them.
